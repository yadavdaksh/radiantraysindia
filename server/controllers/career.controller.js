...
export const adminCreateJob = asyncHandler(async (req, res) => {
  ...
  const job = await prisma.job.create({
    data: {
      ...
      salaryMin: salaryMin && Number(salaryMin) > 0 ? Number(salaryMin) : null,
      salaryMax: salaryMax && Number(salaryMax) > 0 ? Number(salaryMax) : null,
...
export const adminUpdateJob = asyncHandler(async (req, res) => {
  ...
  const job = await prisma.job.update({
    ...
    data: {
      ...
      salaryMin: salaryMin !== undefined ? (salaryMin && Number(salaryMin) > 0 ? Number(salaryMin) : null) : existing.salaryMin,
      salaryMax: salaryMax !== undefined ? (salaryMax && Number(salaryMax) > 0 ? Number(salaryMax) : null) : existing.salaryMax,
